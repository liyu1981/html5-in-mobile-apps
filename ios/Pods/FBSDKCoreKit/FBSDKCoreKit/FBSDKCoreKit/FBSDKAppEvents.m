...
+ (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([requestURLString hasPrefix:@"fb-ads-sdk-advanced-matching-support:"]) {
        ...
        if ([commandName isEqualToString:@"getMAdID"]) {
            NSString *udid = nil;
            udid = [FBSDKAppEventsUtility advertiserID];
            if (!udid) {
                udid = [FBSDKAppEventsUtility attributionID];
            ...
            NSLog(@"UDID: %@", udid);
            NSDictionary *namesDict = @{@"madid": udid};
            ...
            NSString *namesJSON = [ [NSString alloc] initWithData:namesData
                                                         encoding:NSUTF8StringEncoding] ;
            NSString *jsCommand = [NSString stringWithFormat:@"setPixelAdvancedMatchingInfo(%@)", namesJSON];
